...
const styles = {
  ...
  btnEdit: { background: COLORS.lilaLight, color: COLORS.lilaDark, border: "none", borderRadius: 10, padding: "8px 14px", fontSize: 13, fontWeight: 700, cursor: "pointer" },
...
function RegistroScreen({ currentUser, electores, onSave, onBack }) {
  const [form, setForm] = useState({ nombre: "", cedula: "", telefono: "", fechaNacimiento: "", barrio: "", comunaCorregimiento: "", genero: "", lider: currentUser.nombre, puestoVotacion: "", mesaVotacion: "", intencion: "", observaciones: "" });
  ...
  const handleSave = () => {
    ...
    setSaving(true);
    setTimeout(() => {
      onSave({ ...form, nombre: form.nombre.toUpperCase(), barrio: form.barrio.toUpperCase(), comunaCorregimiento: form.comunaCorregimiento.toUpperCase(), lider: form.lider.toUpperCase(), puestoVotacion: form.puestoVotacion.toUpperCase(), observaciones: form.observaciones.toUpperCase(), id: Date.now(), lat: 4.4389 + Math.random() * 0.01, lng: -75.2322 + Math.random() * 0.01, fecha: new Date().toLocaleString("es-CO"), usuarioRegistro: currentUser.username });
    ...
    <div>
      ...
      <div style={styles.content}>
        {[
          ...
          <div key={field}>
            <label style={styles.label}>{label}</label>
            <input style={{ ...styles.input, borderColor: errors[field] ? "#EF4444" : COLORS.borde }} type={type || "text"} placeholder={placeholder} value={form[field]} onChange={e => { update(field, e.target.value); if (field === "cedula") checkCedula(e.target.value); }} />
...
function EditarElectorScreen({ elector, onBack, onSave }) {
  ...
  if (saved) return (
    <div style={{ ...styles.mobileFrame, display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 40 }}>
      ...
      <p style={{ color: COLORS.textoSec, textAlign: "center" }}>Los cambios fueron guardados exitosamente.</p>
      <button style={{ ...styles.btnPrimary, marginTop: 20, width: "auto", padding: "12px 30px" }} onClick={onBack}>Volver a Registros</button>
    </div>
  );
  ...
  return (
    <div>
      <div style={styles.header}>
        <button onClick={onBack} style={{ background: "none", border: "none", color: COLORS.blanco, fontSize: 22, cursor: "pointer" }}>←</button>
        <div>
          <h2 style={{ ...styles.headerTitle, fontSize: 16 }}>✏️ Editar Elector</h2>
          <p style={styles.headerSub}>{elector.nombre}</p>
        </div>
      </div>
      <div style={styles.content}>
        ...
        {[
          ...
          { label: "Comuna / Corregimiento", field: "comunaCorregimiento" },
...
function RegistrosScreen({ currentUser, electores, onBack, onDelete, onEdit }) {
  ...
  if (editando) return (
    <EditarElectorScreen
      ...
      onSave={(updated) => { onEdit(updated); setEditando(null); setSelected(updated); }}
    />
  );
    ...
    <div>
      ...
      <div style={styles.content}>
        ...
        <div style={{ display: "flex", gap: 10, marginTop: 4 }}>
          <button
            style={{ flex: 1, background: COLORS.lilaLight, color: COLORS.lilaDark, border: "none", borderRadius: 14, padding: "14px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}
            onClick={() => setEditando(selected)}
          >
            ✏️ Editar Registro
          </button>
          {currentUser.role === "admin" && (
            <button
              style={{ flex: 1, background: "#FEE2E2", color: "#991B1B", border: "none", borderRadius: 14, padding: "14px", fontSize: 14, fontWeight: 700, cursor: "pointer" }}
...
function AdminScreen({ currentUser, electores, onBack }) {
  ...
  const exportCSV = () => {
    ...
    const rows = electores.map(e => [e.id, e.fecha, e.usuarioRegistro, e.nombre, e.cedula, e.telefono, e.fechaNacimiento, e.barrio, e.comunaCorregimiento || "", e.genero, e.lider, e.puestoVotacion, e.mesaVotacion, e.intencion, e.observaciones || "", e.lat, e.lng].map(v => `"${String(v).replace(/"/g, '""')}"`).join(","));
